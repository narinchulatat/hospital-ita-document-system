...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $status = sanitizeInput($_POST['status'] ?? DOC_STATUS_DRAFT);
        ...
        if (!empty($errors)) {
            ...
            if ($documentId) {
                ...
                $activityLog->log(ACTION_CREATE, 'document', $documentId, 'Uploaded new document: ' . $title);
                
                // Create notification for approvers if status is pending
                ...
                header('Location: ' . BASE_URL . '/staff/documents/view.php?id=' . $documentId . '&uploaded=1');
                exit;
            } else {
                $error = 'เกิดข้อผิดพลาดในการอัปโหลดเอกสาร';
            }
        }
        
    } catch (Exception $e) {
        error_log("Document upload error: " . $e->getMessage());
        $error = 'เกิดข้อผิดพลาดในการอัปโหลดเอกสาร: ' . $e->getMessage();
    }
}
        ...
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">
                    <i class="fas fa-upload mr-3"></i>อัปโหลดเอกสารใหม่
                </h1>
                <p class="text-gray-600 mt-1">เพิ่มเอกสารใหม่เข้าสู่ระบบ</p>
            </div>
        </div>
    </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" 
                                  ...
                                  rows="4"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                  placeholder="อธิบายรายละเอียดของเอกสาร"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center">
                            <input id="status_draft" 
                                   ...
                                   <?= (($_POST['status'] ?? DOC_STATUS_DRAFT) === DOC_STATUS_DRAFT) ? 'checked' : '' ?>
                        ...
                        <div class="flex items-center">
                            <input id="status_pending" 
                                   ...
                                   <?= (($_POST['status'] ?? '') === DOC_STATUS_PENDING) ? 'checked' : '' ?>
                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300">
